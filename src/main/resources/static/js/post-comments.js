document.addEventListener('DOMContentLoaded', () => {
     ...
     * 여러 번 나눠 고른 사진이 누적되게 한다. (여행지 댓글과 같은 방식)
    ...
    form?.addEventListener('submit', async event => {
        ...
        try {
            ...
            await requestJson('/post-comments', {method: 'POST', body});
            form.reset();
            picker?.clear();
    ...
    list.addEventListener('click', async event => {
        ...
        if (event.target.matches('.post-comment-reply-button')) {
            removeReplyForm(list.querySelector('.post-comment-reply-form'));
            ...
            const inputId = `post-comment-reply-image-${commentId}`;
            ...
            actions.append(imageInput, uploadLabel, submit,
                makeButton('취소', 'post-comment-reply-cancel'));
        ...
        if (event.target.matches('.post-comment-reply-cancel')) {
            removeReplyForm(event.target.closest('.post-comment-reply-form'));
            return;
        }
    ...
    /**
     * 댓글 사진 클릭 시 확대 모달.
     * 이동은 클릭한 댓글에 첨부된 사진 안에서만 순환한다. (여행지 댓글과 같은 방식)
     */
    function initCommentImageModal() {
        const modal = document.getElementById('post-comment-image-modal');
        const modalImage = document.getElementById('post-comment-modal-img');
